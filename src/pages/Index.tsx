...
import { Bomb, RefreshCw, Dice5, ShieldCheck, Zap, LockKeyhole, Timer, UserCheck, X, CheckCircle2, Share, PlusSquare, Smartphone, Copy, Check } from "lucide-react";
            ...
            <div className="bg-white rounded-[3rem] border border-slate-50 shadow-2xl overflow-hidden">
               <div className="p-8 md:p-12 bg-slate-950 text-white flex flex-col md:flex-row justify-between items-center gap-6">
                  <div className="text-center md:text-left flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-center md:justify-start gap-3 group cursor-pointer" onClick={handleCopy}>
                      <p className="text-xl md:text-4xl font-mono font-bold break-all group-hover:text-blue-400 transition-colors">{currentEmail}</p>
                      <div className="bg-white/10 p-2 rounded-xl group-hover:bg-blue-600 transition-all">
                        {isCopied ? <Check className="w-5 h-5 text-green-400" /> : <Copy className="w-5 h-5" />}
                      </div>
                    </div>
                  </div>
                  <Button onClick={() => refetch()} className="bg-blue-600 hover:bg-blue-500 h-14 px-8 rounded-2xl font-black shadow-lg shrink-0">
        ...
        {!currentEmail && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-20">
            <div className="bg-blue-600 rounded-[3rem] p-10 md:p-14 text-white shadow-2xl shadow-blue-600/20 flex flex-col justify-center relative overflow-hidden">
              ...
              <p className="text-blue-100 font-medium mb-8 leading-relaxed">Add <b>pandeykapil.com.np</b> to your home screen.</p>
            ...
            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-xl flex flex-col justify-center">
              ...
              <div className="space-y-8">
                <div className="flex items-center gap-6 group"><div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-blue-600 border border-slate-100">1</div><p className="font-bold text-slate-700">Tap the <Share className="w-5 h-5 inline mx-1 text-blue-600" /> <b>Share</b> button</p></div>
                <div className="flex items-center gap-6 group"><div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-blue-600 border border-slate-100">2</div><p className="font-bold text-slate-700">Tap <PlusSquare className="w-5 h-5 inline mx-1 text-blue-600" /> <b>Add to Home Screen</b></p></div>
                <div className="flex items-center gap-6 group"><div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-blue-600 border border-slate-100">3</div><p className="font-bold text-slate-700">Tap <b>Add</b> in top right</p></div>
              </div>
            </div>
          </div>
        )}
